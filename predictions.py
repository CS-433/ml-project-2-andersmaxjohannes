...
modelname = './modelsave'
loadmodel = torch.load(modelname)
classes = ['Background', 'Cube', 'Octahedron', 'Sphere']

box_thresh=0.1  # The minimum confidence level for a bounding box to be included as a prediction. In the range [0,1]
...
for i in range(len(images)):
    ...
    pred_boxes_i, pred_masks_i, pred_labels_i, instances_i = predict(loadmodel,os.path.join(img_dir,images[i]), classes, printout=printout, show_img=show_img, img_savefile=imgSavefile, device=device)
